...
const prefix = "thread017";
const instrument = "kantele";
// const rawsoundfiles = ["kantele1", "kantele2", "kantele3"];
const rawsoundfiles = ["kantele1", "kantele3"];
...
let catalog = rawsoundfiles.reduce( (catalog, rawsoundfile) => {
  ...
  catalog[rawsoundfile] = [...Array(nthreads).keys()].reduce( (threads,j) => {
    ...
    while(dur < threadlength) {
      ...
      let tonepad = tools.randominteger(0,100)/100;
      tones.push( [ tone, tonefile+".mp3",  tonepad]);

      let tone2 = notes[tools.randominteger(0,nnotes)];
      let tonefile2 = rawsoundfile+"_"+tone;
      let tonepad2 = tools.randominteger(0,100)/100;
      tones2.push( [ tone2, tonefile+".mp3",  tonepad]);

      dur = dur + rawsoundfiledata.filter(f => f.id===tonefile)[0].duration + tonepad;

      let bendtone = tools.randominteger(80,160)/100;
      let bendtonefile = rawsoundfile+"_bend_" + bendtone;
      let bendpad = tools.randominteger(0,100)/100;
      ...
      threadstr = threadstr + ` "|sox ${tonefile}.mp3 -p pad 0 ${tonepad}" `;
      threadstr2 = threadstr2 + ` "|sox ${tonefile2}.mp3 -p pad 0 ${tonepad2}" `;
    ...
    threads.push( [ tones2, threadstr2 + " " + rawsoundfile + "_thread2_" + j.toString().padStart(3, "0") + ".mp3 echos " + echos[(j+1)%echos.length] + "  norm -2"  ] );
    threads.push( [ bendtones, bendthreadstr + " " + rawsoundfile + "_bendthread_" + j.toString().padStart(3, "0") + ".mp3 echos " + echos[(j+2)%echos.length] + "  norm -2"  ] );
...
if(rawsoundfiles.length>1) {
  catalog["mixed"] = [...Array(nthreads).keys()].reduce( (threads,j) => {
      ...
      while(dur < threadlength) {
        ...
        let tonefile2 = rawsoundfile+"_"+tone;
        let tonepad2 = tools.randominteger(0,100)/100;
        tones2.push( [ tone2, tonefile2 + ".mp3",  tonepad]);
      ...
      threads.push( [ tones, threadstr + " mixed_thread_" + j.toString().padStart(3, "0") + ".mp3 echos " + echos[j%echos.length] + "  norm -2"  ] );
      threads.push( [ tones2, threadstr2 + " " + rawsoundfile + "_thread2_" + j.toString().padStart(3, "0") + ".mp3 echos " + echos[(j+1)%echos.length] + "  norm -2"  ] );
      threads.push( [ bendtones, bendthreadstr + " mixed_bendthread_" + j.toString().padStart(3, "0") + ".mp3 echos " + echos[(j+2)%echos.length] + "  norm -2"  ] );

      //no echo
      // threads.push( [ tones, threadstr + " mixed_thread_" + j.toString().padStart(3, "0") + ".mp3 echos   norm -2"  ] );
      // threads.push( [ bendtones, bendthreadstr + " mixed_bendthread_" + j.toString().padStart(3, "0") + ".mp3 echos  norm -2"  ] );
