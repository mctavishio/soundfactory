...
//for file in ?(*.mp3|*.wav); do soxi -D $file | read d ; soxi -c $file | read c ; soxi -r $file | read r ; soxi -t $file | read t ; soxi -p $file | read p ;echo {id:\"$file\",file:\"$file\", duration:$d, nchannels:$c, rate:$r, type:\"$t\", bitrate:$p}, >> soundfiles.js; done;
//add module.exports = [ ] & replace .mp3",file: with ",file:
const rawsoundfiledata = require("./soundfiles.js");
const prefix = "thread014";
const instrument = "icedbowedvibes";
const rawsoundfiles = ["icebowedvibes1"];
...
let catalog = rawsoundfiles.reduce( (catalog, rawsoundfile) => {
  ...
  catalog[rawsoundfile] = [...Array(nthreads).keys()].reduce( (threads,j) => {
    ...
    while(dur < threadlength) {
      ...
      // let bendtone = tools.randominteger(84,116)/100;
...
catalog["mixed"] = [...Array(nthreads).keys()].reduce( (threads,j) => {
    ...
    while(dur < threadlength) {
      ...
      threadstr = threadstr + " " + tonefile + ".mp3";
      // bendthreadstr = bendthreadstr + ` "|sox ${rawsoundfile}.mp3 -p speed ${tools.randominteger(90)/110} norm -4" `;
      bendthreadstr = bendthreadstr + ` "|sox ${tonefile}.mp3 -p speed ${bendtone} norm -4" `;
    }
    //with echo
...
// play -M "|sox bell11_harmonictwist_all_fm.mp3 -c1 -p" "|sox -v 1.0 bell11_bendtwist_all_fm.mp3 -c1 -p" remix 1v0.7,2v0.3 1v0.3,2v0.7 norm -2
  ...
  //  gsutil -m cp -r icedbowedvibes_1641489730425 gs://soundfactory/
