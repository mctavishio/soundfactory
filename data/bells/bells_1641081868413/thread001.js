...
const echos = [...Array(nthreads).keys()].map( j => {
  ...
  let delay = [tools.randominteger(80,1200),tools.randominteger(3400,4400)];
  let decay = [tools.randominteger(3,6)/10,tools.randominteger(1,3)/10];
...
nextsteps = nextsteps + rawsoundfiles.reduce( (nextstepstr,file1) => {
  rawsoundfiles.forEach( file2 => {
    nextstepstr = nextstepstr + `
    sox -M "|sox  -v 0.8 ${file1}_harmonictwist_all_fm.mp3 -c1 -p" "|sox -v 0.8 ${file2}_harmonictwist_all_fm.mp3 -c1 -p" ${file1}_${file2}_harmonictwist_all_fm.mp3 remix 1v0.7,2v0.3 1v0.3,2v0.7 norm -2;
    sox -M "|sox  -v 0.8 ${file1}_bendtwist_all_fm.mp3 -c1 -p" "|sox -v 0.8 ${file2}_bendtwist_all_fm.mp3 -c1 -p" ${file1}_${file2}_bendtwist_all_fm.mp3 remix 1v0.7,2v0.3 1v0.3,2v0.7 norm -2;
    sox -M "|sox  -v 0.8 ${file1}_harmonictwist_all_fm.mp3 -c1 -p" "|sox -v 0.8 ${file2}_bendtwist_all_fm.mp3 -c1 -p" ${file1}_${file2}_twist_all_fm.mp3 remix 1v0.7,2v0.3 1v0.3,2v0.7 norm -2;
    `
  })
  return nextstepstr;
}, "");
