...
const rawsoundfiles = ["bell11", "bell13", "bell6", "longbell"];
...
let bendcatalog = rawsoundfiles.reduce( (catalog, rawsoundfile) => {
  ...
  catalog[rawsoundfile] = [...Array(nthreads).keys()].reduce( (threads,j) => {
    ...
    while(dur < threadlength) {
      ...
      let tonefile = rawsoundfile+"_bend_"+tone;
      tones.push( [ "bend_"+tone, tonefile+".mp3" ]);
      threadstr = threadstr + ` "|sox ${rawsoundfile}.mp3 -p speed ${tone} norm -4" `;
      // console.log(`dur=${dur}`);
      dur = dur + rawsoundfiledata.filter(f => f.id===rawsoundfile)[0].duration*tone;
      // threadstr = threadstr + " " + tonefile+".mp3";
    }
    //with echo
...
bendcatalog["mixed"] = [...Array(nthreads).keys()].reduce( (threads,j) => {
  let threadstr="sox ";
  let dur = 0;
  let tones = [];
  while(dur < threadlength) {
  ...
  //with echo
  threads.push( [ tones, threadstr + " mixed_bendthread_" + j.toString().padStart(3, "0") + ".mp3 echos " + echos[(j+1)%echos.length] + " norm -2 silence -l 1 0.1 1% -1 2.0 1%"  ] );
  //no echo
  // threads.push( [ thread, threadstr + " " + rawsoundfile + "_bendthread_" + j.toString().padStart(3, "0") + ".mp3  trim 0 "+threadlength+" fade 0 -0 12 norm -2"  ] );
  return threads;
}, []);
...
rawsoundfiles.forEach( file => {
  ...
  let score = `module.exports = {
    ...
    harmonicthreads: ${JSON.stringify(catalog[file])},
    bendthreads: ${JSON.stringify(bendcatalog[file])},
...
( file => {
  ...
  let score = `module.exports = {
    ...
    nthreads: ${nthreads},
    duration: ${threadlength},
    harmonicthreads: ${JSON.stringify(catalog["mixed"])},
    bendthreads: ${JSON.stringify(bendcatalog["mixed"])},
    echos: ${JSON.stringify(echos)},
  }`
  fs.writeFileSync(scorefilename, score, (err) => {
    if (err)
