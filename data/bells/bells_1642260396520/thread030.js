...
const prefix = "thread030";
const instrument = "bells";
// const rawsoundfiles = ["bell11", "bell2", "bell6", "longbell"];
const rawsoundfiles = ["bell11", "bell2", "bell13"];
...
const intervals = {
    ...
    noise: basetone => { return basetone*tools.randominteger(5,20)/10 },
...
//     return { id, notes };
//   } ) ("harmonic", { lowi: 0, bassi: 1, bassV: 2, bassIV: 3, I: 6, II: 5, majIII: 2, miniii: 5, IV: 9, V: 9, VI: 3, majVII: 1, minvii: 6, VIII: 1, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
// );
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("pentatonic2", { lowi: 1, bassi: 2, bassV: 1, bassIV: 1, I: 6, II: 3, majIII: 0, miniii: 0, IV: 6, V: 6, VI: 6, majVII: 0, minvii: 0, VIII: 3, lownoise: 1, midnoise: 3, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("simple", { lowi: 1, bassi: 0, bassV: 1, bassIV: 1, I: 6, II: 0, majIII: 0, miniii: 0, IV: 6, V: 6, VI: 0, majVII: 0, minvii: 0, VIII: 3, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
...
//     let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
//     return { id, notes };
//   } ) ("buzz", { lowi: 0, bassi: 0, bassV: 0, bassIV: 0, I: 2, II: 0, majIII: 0, miniii: 0, IV: 0, V: 0, VI: 0, majVII: 0, minvii: 0, VIII: 0, lownoise: 1, midnoise: 1, highnoise: 1, noise: 1, buzz: 8 })
// );
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("noise", { lowi: 0, bassi: 0, bassV: 1, bassIV: 0, I: 2, II: 0, majIII: 0, miniii: 0, IV: 0, V: 0, VI: 0, majVII: 0, minvii: 2, VIII: 0, lownoise: 2, midnoise: 2, highnoise: 2, noise: 4, buzz: 1 })
);
  ...
  catalog[rawsoundfile] = [...Array(nthreads).keys()].reduce( (threads,j) => {
    ...
    threadp.forEach( (p ,k) => {
      ...
      while(dur < threadlength) {
        ...
        let tonepad = tools.randominteger(0,0)/100;
