...
const soundfiles = require("./outsoundfiles.js");
const rawsoundfiles = ["bagpipe1e", "bagpipe1f", "bagpipe1g", "bagpipe1h"];
const _timestamp = 1642440127604;
const _date = new Date(_timestamp).toString();
const _title = `bagpipeclick_${_timestamp}_reverb`; 
const _root = `https://soundfactory.work/data/`; 
const _url = `${_root}${_title}`;
const _abstract = `sound file index: generative output from ${_title}`;
const _pictureurl = "/apple-touch-icon.png";
const _cssurl = "/css/soundfactory.css";
const _codeurl = "";
...
fs.writeFileSync("index.html", head+body, (err) => {
  if (err)
    console.log(err);
  else {
