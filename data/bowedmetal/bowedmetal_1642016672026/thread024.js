...
const prefix = "thread024";
const instrument = "bowedmetal";
const rawsoundfiles = ["icebowedcymbal2", "icebowedvibes1"]
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("pentatonic", { lowi: 0, bassi: 1, bassV: 2, bassIV: 2, I: 6, II: 0, majIII: 0, miniii: 6, IV: 6, V: 6, VI: 0, majVII: 0, minvii: 5, VIII: 2, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("pentatonic2", { lowi: 0, bassi: 1, bassV: 2, bassIV: 2, I: 6, II: 3, majIII: 0, miniii: 0, IV: 6, V: 6, VI: 6, majVII: 0, minvii: 0, VIII: 1, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("simple", { lowi: 0, bassi: 1, bassV: 2, bassIV: 2, I: 6, II: 0, majIII: 0, miniii: 0, IV: 6, V: 6, VI: 0, majVII: 0, minvii: 0, VIII: 3, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
  ...
  catalog[rawsoundfile] = [...Array(nthreads).keys()].reduce( (threads,j) => {
    ...
    threadp.forEach( (p ,k) => {
      ...
      while(dur < threadlength) {
        ...
        let tonepad = tools.randominteger(20,200)/100;
...
nextsteps = nextsteps + rawsoundfiles.reduce( (nextstepstr,file1) => {
  rawsoundfiles.forEach( file2 => {
    ...
    [...Array(Math.floor(nthreads/2)).keys()].forEach( k1 => {
        [...Array(Math.floor(nthreads/2)).keys()].map( t => nthreads-t-1 ).forEach( k2 => {
        nextstepstr = nextstepstr + `
