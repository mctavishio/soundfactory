...
const prefix = "thread019";
const instrument = "clarinet";
...
const intervals = {
    ...
    lownoise: basetone => { return basetone*tools.randominteger(5,12)/10 },
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    let speeds = Object.entries(intervals).reduce( (acc,entry) => {
      acc[entry[0]] = (entry[1](1000)/1000).toFixed(3);
      return acc;
    }, {});
    ...
    return { id, notes };
  } ) ("harmonic", { lowi: 1, bassi: 1, bassV: 2, bassIV: 3, I: 6, II: 5, majIII: 2, miniii: 5, IV: 9, V: 9, VI: 3, majVII: 1, minvii: 6, VIII: 1, lownoise: 1, midnoise: 1, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("pentatonic", { lowi: 1, bassi: 1, bassV: 2, bassIV: 2, I: 6, II: 0, majIII: 0, miniii: 6, IV: 6, V: 6, VI: 0, majVII: 0, minvii: 5, VIII: 1, lownoise: 1, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("pentatonic2", { lowi: 1, bassi: 1, bassV: 2, bassIV: 2, I: 6, II: 6, majIII: 0, miniii: 0, IV: 6, V: 6, VI: 6, majVII: 0, minvii: 0, VIII: 1, lownoise: 1, midnoise: 1, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("simple", { lowi: 1, bassi: 1, bassV: 2, bassIV: 2, I: 6, II: 0, majIII: 0, miniii: 0, IV: 6, V: 6, VI: 0, majVII: 0, minvii: 0, VIII: 6, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
...
nextsteps = nextsteps + rawsoundfiles.reduce( (nextstepstr,file1) => {
  rawsoundfiles.forEach( file2 => {
    ...
    [...Array(Math.floor(nthreads/4)).keys()].forEach( k1 => {
        [...Array(Math.floor(nthreads/4)).keys()].map( t => nthreads.length-t ).forEach( k2 => {
        nextstepstr = nextstepstr + `
