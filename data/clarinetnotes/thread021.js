...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("pentatonic2", { lowi: 0, bassi: 1, bassV: 2, bassIV: 2, I: 6, II: 6, majIII: 0, miniii: 0, IV: 6, V: 6, VI: 6, majVII: 0, minvii: 0, VIII: 1, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("simple", { lowi: 0, bassi: 1, bassV: 2, bassIV: 2, I: 6, II: 0, majIII: 0, miniii: 0, IV: 6, V: 6, VI: 0, majVII: 0, minvii: 0, VIII: 4, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
...
let nextsteps = rawsoundfiles.reduce( (nextstepstr,file) => {
  nextstepstr = nextstepstr + `
    ...
    mv ${file}_bentthread_*.mp3 ${newdir};
