...
const prefix = "thread052";
const instrument = "tones";
const rawsoundfiles = ["128_c", "144_d", "192_g", "288_d", "384_g", "432_a", "64_c"];
...
const intervals = {
    ...
    lownoise: basetone => { return basetone*tools.randominteger(8,10)/10 },
    midnoise: basetone => { return basetone*tools.randominteger(8,11)/10 },
    highnoise: basetone => { return basetone*tools.randominteger(12,20)/10 },
    noise: basetone => { return basetone*tools.randominteger(9,20)/10 },
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    return { id, notes };
  } ) ("harmonic", { lowi: 0, bassi: 0, bassV: 0, bassIV: 0, I: 6, II: 5, majIII: 1, miniii: 5, IV: 9, V: 9, VI: 2, majVII: 1, minvii: 4, VIII: 2, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("pentatonic", { lowi: 0, bassi: 0, bassV: 0, bassIV: 0, I: 6, II: 0, majIII: 0, miniii: 6, IV: 6, V: 6, VI: 0, majVII: 0, minvii: 2, VIII: 1, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("pentatonic2", { lowi: 0, bassi: 0, bassV: 0, bassIV: 0, I: 6, II: 3, majIII: 0, miniii: 0, IV: 6, V: 6, VI: 6, majVII: 0, minvii: 0, VIII: 2, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("simple", { lowi: 0, bassi: 0, bassV: 0, bassIV: 0, I: 6, II: 0, majIII: 0, miniii: 0, IV: 6, V: 6, VI: 0, majVII: 0, minvii: 0, VIII: 1, lownoise: 0, midnoise: 0, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("buzz", { lowi: 0, bassi: 0, bassV: 0, bassIV: 0, I: 2, II: 0, majIII: 0, miniii: 0, IV: 1, V: 1, VI: 0, majVII: 0, minvii: 1, VIII: 1, lownoise: 1, midnoise: 1, highnoise: 1, noise: 1, buzz: 8 })
);
...
//     let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
//     return { id, notes };
//   } ) ("simplebuzz", { lowi: 0, bassi: 0, bassV: 2, bassIV: 2, I: 0, II: 0, majIII: 0, miniii: 0, IV: 0, V: 0, VI: 0, majVII: 0, minvii: 1, VIII: 0, lownoise: 0, midnoise: 1, highnoise: 1, noise: 0, buzz: 8 })
// );
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("noise", { lowi: 0, bassi: 0, bassV: 2, bassIV: 2, I: 2, II: 0, majIII: 0, miniii: 0, IV: 0, V: 0, VI: 0, majVII: 0, minvii: 2, VIII: 0, lownoise: 0, midnoise: 2, highnoise: 2, noise: 4, buzz: 2 })
);
...
const mcompandstr = `reverb 100 50 50 gain -7 sinc -n 29 -b 100 8000 mcompand "0.005,0.1 -47,-40,-34,-34,-17,-33" 100 "0.003,0.05 -47,-40,-34,-34,-17,-33" 400 "0.000625,0.0125 -47,-40,-34,-34,-15,-33" 1600 "0.0001,0.025 -47,-40,-34,-34,-31,-31,-0,-30" 6400 "0,0.025 -38,-31,-28,-28,-0,-25" gain 15 highpass 22 highpass 22 sinc -n 255 -b 16 -17500 gain 6 lowpass -1 17801`;
  ...
  catalog[rawsoundfile] = [...Array(nthreads).keys()].reduce( (threads,j) => {
    ...
    threadp.forEach( (p ,k) => {
      ...
      while(dur < threadlength) {
        ...
        let tonepad = tools.randominteger(0,10)/100;
...
nextsteps = nextsteps + rawsoundfiles.reduce( (nextstepstr,file1) => {
  rawsoundfiles.forEach( file2 => {
    threadp.forEach( (p,j) => {
    nextstepstr = nextstepstr + `
    sox -M "|sox  -v 0.7 ${file1}_thread_${p.id}_all.mp3 -c1 -p pad 1.3 0 norm -4" "|sox -v 0.7 ${file2}_thread_${p.id}_all.mp3 -c1 -p norm -6" ${file1}_${file2}_thread_${p.id}_all.mp3 remix 1v0.7,2v0.3 1v0.3,2v0.7  ${mcompandstr};
    `;
    [...Array(Math.floor(nthreads/4)).keys()].forEach( k1 => {
        [...Array(Math.floor(nthreads/4)).keys()].map( t => nthreads-t-1 ).forEach( k2 => {
        nextstepstr = nextstepstr + `
    sox -M "|sox  -v 0.7 ${file1}_thread_${p.id}_${k1.toString().padStart(3, "0")}.mp3 -c1 -p pad 1.3 0 norm -6" "|sox -v 0.7 ${file2}_thread_${p.id}_${k2.toString().padStart(3, "0")}.mp3 -c1 -p norm -6" ${file1}_${file2}_thread_${p.id}_${k1.toString().padStart(3, "0")}_${k2.toString().padStart(3, "0")}.mp3 remix 1v0.7,2v0.3 1v0.3,2v0.7 ${mcompandstr};
    `
    });
    });
    });
