...
const prefix = "thread053";
const instrument = "clarinetgeese";
const rawsoundfiles = ["clarinet2", "clarinetjazz0b", "clarinetnotes_e"];
...
nextsteps = nextsteps + rawsoundfiles.reduce( (nextstepstr,file1) => {
  rawsoundfiles.forEach( file2 => {
    threadp.forEach( (p,j) => {
    nextstepstr = nextstepstr + `
    sox -M "|sox  -v 0.7 ${file1}_thread_${p.id}_all.mp3 -c1 -p pad 1.3 0 norm -6" "|sox -v 0.7 ${file2}_thread_${p.id}_all.mp3 -c1 -p norm -6" ${file1}_${file2}_thread_${p.id}_all.mp3 remix 1v0.7,2v0.3 1v0.3,2v0.7  ${mcompandstr};
    `;
