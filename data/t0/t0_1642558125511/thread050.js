...
const prefix = "thread050";
const instrument = "t0";
const rawsoundfiles = ["t0"];
...
const intervals = {
    ...
    noise: basetone => { return basetone*tools.randominteger(6,12)/10 },
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("pentatonic", { lowi: 0, bassi: 0, bassV: 0, bassIV: 0, I: 6, II: 0, majIII: 0, miniii: 6, IV: 6, V: 6, VI: 0, majVII: 0, minvii: 2, VIII: 1, lownoise: 0, midnoise: 1, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("pentatonic2", { lowi: 0, bassi: 0, bassV: 0, bassIV: 0, I: 6, II: 3, majIII: 0, miniii: 0, IV: 6, V: 6, VI: 6, majVII: 0, minvii: 0, VIII: 2, lownoise: 0, midnoise: 1, highnoise: 0, noise:0, buzz: 0 })
);
...
threadp.push( 
  ( ( id, harmonicweights ) => {
    ...
    let notes = tools.reifyWeightedArray( Object.entries(weights).map( w=>{ return [w[0],Math.floor(w[1])] } ) );
    return { id, notes };
  } ) ("buzz", { lowi: 0, bassi: 0, bassV: 0, bassIV: 0, I: 2, II: 0, majIII: 0, miniii: 0, IV: 1, V: 1, VI: 0, majVII: 0, minvii: 1, VIII: 1, lownoise: 0, midnoise: 1, highnoise: 1, noise: 1, buzz: 8 })
);
  ...
  catalog[rawsoundfile] = [...Array(nthreads).keys()].reduce( (threads,j) => {
    ...
    threadp.forEach( (p ,k) => {
      ...
      while(dur < threadlength) {
        ...
        let tonepad = tools.randominteger(0,40)/100;
